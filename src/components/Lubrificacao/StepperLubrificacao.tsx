      ...
      <List>
        {props.lubMachine?.procedures?.map((procedure) => (
          <ListItem
            className={clsx(
              classes.listItem,
              !procedure.checked && classes.uncheckedListItem
            )}
            key={`procedures-review-${procedure._id}`}
          >
            <ListItemText
              primary={procedure.description}
              secondary={procedure.method}
            />
...
interface PropsStepperLimpezaInspecao {
  ...
  setSelectedLubMachine: React.Dispatch<
    React.SetStateAction<LubMachine | undefined>
  >;
  className: string;
  sendReportToServer: () => void;
  handleReportComments: (
...
const StepperLimpezaInspecao: FC<AllProps> = (props: AllProps) => {
  ...
  const [lineIsUp, setLineIsUp] = React.useState(true);
  const steps = getSteps(props.selectedLubMachine?.procedures as []);
  const history = useHistory();

  const handleNext = () => {
    if (activeStep >= steps.length - 1 || lineIsUp === false) {
      ...
      <React.Fragment>
        {slicedProcedures?.map((procedure) => (
          <Container
            ...
            key={`lubProcedure-${procedure._id}`}
          >
            <Accordion
              className={
                procedure.checked ? classes.markedAccordion : undefined
              }
            >
  ...
  return (
    <Paper className={props.className}>
      <div className={classes.stepperRoot}>
        ...
        {String(props?.selectedLubMachine?.period).toLowerCase() === 'diário'||String(props?.selectedLubMachine?.period).toLowerCase() === 'diario'||String(props?.selectedLubMachine?.period).toLowerCase().localeCompare('turno')?
        <Container
          style={{
            display: "flex",
        ...
        >
          ...
          <Checkbox
            color="primary"
            onChange={() => {
              setLineIsUp(!lineIsUp);
              const lubMachine = props?.selectedLubMachine
              if(lubMachine){
                lubMachine.lineWasUp = !lineIsUp
                props?.setSelectedLubMachine(lubMachine)
              }
            }}
          />
        </Container>
        ...
        {steps.map((step, stepIndex) => (
          <div
            ...
            className={clsx(
              classes.container,
              activeStep !== stepIndex && classes.hide
            )}
          >
            {ProceduresContent(
              stepIndex,
        ...
        <div>
          {activeStep === steps?.length - 1 || lineIsUp === false ? (
            <Container>
              <StepperSummary
                lubMachine={props.selectedLubMachine as LubMachine}
              />
