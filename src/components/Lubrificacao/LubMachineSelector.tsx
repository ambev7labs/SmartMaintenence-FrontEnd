...
interface PropsLubMachineSelector {
  ...
  setLubMachinesDoGrupo: React.Dispatch<
...
const LubMachineSelector: FC<AllProps> = (props: AllProps) => {
  ...
  useEffect(() => {
    axios
      .get(
        `/lubEquip/indexbyequip?field=${userData.user?.field}&group=${props.grupo}`
      )
      .then((response) => {
  ...
  const handleChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    ...
    props.setSelectedLubMachine(
      props.lubMachinesDoGrupo?.filter(
  ...
  return (
    <FormControl className={classes.selector} variant="outlined">
      <Select
        value={props.selectedLubMachineId}
      ...
      >
        ...
        {props.lubMachinesDoGrupo?.length !== 0 ? (
          props.lubMachinesDoGrupo?.map((lubMachine, index) => (
            <MenuItem key={"lubMachine-" + index} value={lubMachine._id}>
              {lubMachine.name +
                ...
                lubMachine.period +
                " " +
                lubMachine.frequency +
                "x)"}
            </MenuItem>
          ))
        ) : props.grupo ? (
          <MenuItem key={"equipamento-concluído"} value="" disabled>
      ...
      <FormHelperText>
        Selecione um Equipamento para Lubrificação
      </FormHelperText>
    </FormControl>
  );
};
