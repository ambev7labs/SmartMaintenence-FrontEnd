      ...
      <React.Fragment>
        {slicedProcedures?.map((procedure) => (
          <Container
            ...
            key={`procedimento-${procedure._id}`}
          >
            <Accordion
              disabled={lineIsUp ? false : true}
              className={
  ...
  return (
    <Paper className={props.className}>
      <div className={classes.stepperRoot}>
        ...
        {String(props?.selectedEquipamento?.period).toLowerCase() ===
          "diário" ||
        String(props?.selectedEquipamento?.period).toLowerCase() === "diario" ||
        String(props?.selectedEquipamento?.period).toLowerCase() == "turno" ? (
          <Container
            style={{
              display: "flex",
          ...
          >
            ...
            <Checkbox
              ...
              onChange={() => {
                ...
                const equipamento = props?.selectedEquipamento;
                if (equipamento) {
                  equipamento.lineWasUp = !lineIsUp;
                  props?.setSelectedEquipamento(equipamento);
                }
              }}
            />
          </Container>
        ...
        <div>
          {activeStep === steps?.length - 1 || lineIsUp === false ? (
            <Container>
              {props.selectedEquipamento?.lineWasUp ? (
                <StepperSummary
                  equipamento={props.selectedEquipamento as Equipamento}
                />
              ) : (
                ""
              )}
